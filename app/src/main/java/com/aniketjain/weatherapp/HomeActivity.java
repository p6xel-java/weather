    ...
    private void getTodayWeatherInfo(String name) {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, url.getLink(), null, response -> {
            try {
                ...
                condition = weatherObj.getInt("id");
                ...
                sunrise = sysObj.getLong("sunrise");
                sunset = sysObj.getLong("sunset");

                // Temperature data
                JSONObject mainObj = response.getJSONObject("main");
                double tempKelvin = mainObj.getDouble("temp");
                double feelsLikeTempKelvin = mainObj.getDouble("feels_like");
                double tempMinKelvin = mainObj.getDouble("temp_min");
                double tempMaxKelvin = mainObj.getDouble("temp_max");

                // Convert temperatures to Celsius
                temperature = String.valueOf(Math.round(tempKelvin - 273.15));
                min_temperature = String.valueOf(Math.round(tempMinKelvin - 273.15));
                max_temperature = String.valueOf(Math.round(tempMaxKelvin - 273.15));
                ...
                wind_speed = String.valueOf(windObj.getDouble("speed"));
